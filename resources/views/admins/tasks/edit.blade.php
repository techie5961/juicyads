...
@section('main')
    ...
    <section class="column p-10 w-full g-10">
        <form action="{{ url('admins/post/task/edit/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.Posted)" class="w-full active bg-white br-10 box-shadow p-10 column g-10">
            <input type="hidden" name="_token" class="input" value="{{ @csrf_token() }}">
            <input type="hidden" name="id" class="input" value="{{ $task->id }}">
            <div class="row w-full g-10 align-center">
                ...
                <strong class="desc c-bg-secondary">Edit {{ $task->uniqid }}</strong>
            </div>
            <hr>
            {{-- NEW INPUT --}}
             ...
             <select onchange="MyFunc.CategorySelected(this)" name="category" class="w-full required h-full bg-transparent br-10 no-border inp input">
               ...
               @else
                   @foreach ($categories as $data)
                       <option {{ $task->category->id == $data->id ? 'selected' : '' }} value="{{ $data->id }}">{{ ucfirst($data->name) }}</option>
                   @endforeach
               @endif
            </select>  
            </div>
            ...
            <div class="w-full  cont pc-pointer h-50 border-1 br-5 row align-center space-between bg-dim border-color-silver">
             <select onchange="
                ...
                @if ($subcategories->isEmpty())
                   <option value="" disabled>No subcategory available for this category</option>
      @else
                     <option value="" disabled>Select Subcategory...</option>
                   @foreach ($subcategories as $data)
                       <option {{ $task->subcategory->id == $data->id ? 'selected' : '' }} value="{{ $data->id }}">{{ ucwords($data->name) }}</option>
                   @endforeach
     @endif
            </select>  
            </div>
            ...
            <label for="">Task Link</label>
             <div class="w-full cont pc-pointer h-50 border-1 br-5 row align-center space-between bg-dim border-color-silver">
            <input value="{{ $task->link }}" placeholder="Enter task url/link" name="link" type="url" class="inp required input w-full h-full br-10 bg-transparent no-border"> 
            </div>
             </div>
              {{-- NEW INPUT --}}
             <div class="column w-full g-5">
             ...
             <div class="w-full cont pc-pointer h-50 border-1 br-5 row align-center space-between bg-dim border-color-silver">
            <input value="{{ $task->limit }}" placeholder="Enter task limit" name="limit" type="number" class="inp required input w-full h-full br-10 bg-transparent no-border"> 
            </div>
             </div>
            <button class="post bg-secondary secondary-text"><span>Save Changes</span></button>
        </form>
