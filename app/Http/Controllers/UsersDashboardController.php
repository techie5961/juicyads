  ...
  $transactions=DB::table('transactions')->where('user_id',Auth::guard('users')->user()->id)->orderBy('date','desc')->paginate(5);
    ...
    public function Tasks(){
        ...
        $tasks->getCollection()->transform(function($each){
            ...
            $each->subcat=json_decode($each->subcategory);
            $each->subcategory=DB::table('subcategories')->where('id',$each->subcat->id)->first() ?? $each->subcat;
            $each->title=$each->category->name.' | '.$each->subcategory->name;
            return $each;
        });
