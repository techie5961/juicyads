      ...
      {{-- NEW DIV--}}
        ...
        @else
            @foreach ($subcategories as $data)
                <div class="column g-10 w-full p-10 br-10 box-shadow bg-light">
                  <div class="w-full row g-10">
                  <img class="h-70 perfect-square br-10" src="{{ asset('subcategories/'.$data->icon.'') }}" alt="">
                  <div class="column g-10">
                    <strong class="desc">{{ ucfirst($data->category->name) }} || {{ ucwords($data->name) }}</strong>
                  ...
                  <div class="m-left-auto row align-center g-10">
                    ...
                    <div onclick="window.location.href='{{ url('admins/subcategories/edit?id='.$data->id.'') }}'" class="h-50 min-w-50 br-10 clip-10 bg-blue pointer no-select c-white p-10 column justify-center"> Edit </div>
                  </div>
                </div>
                <hr>
                <div class="bold row align-center g-10 font-1">
                  <span>Category :</span>
                  <img src="{{ asset('categories/'.$data->category->icon.'') }}" alt="" class="h-20 perfect-square circle">
                  <span>{{ ucfirst($data->category->name) }}</span>
                </div>
                <div class="bold font-1">
                  <span>Reward :</span>
                  <span>&#8358;{{ number_format($data->reward) }}</span>
                </div>
               @isset($data->instructions)
               <strong class="desc c-primary u">Instructions</strong>
                  <div style="background: rgba(0,0,0,0.05)" class="w-full p-10 br-10">
            ...
            @if ($subcategories->hasMorePages())
                @include('components.sections',[
                  ...
                  'page' => $subcategories->currentPage() + 1
                ])
            @endif
        @endif
...
<span>Are you sure you want to delete this SubCategory?<br>This action cannot be undone</span>
<button onclick="window.location.href='{{ url('admins/get/subcategory/delete') }}?id=${id}'" class="btn-red w-full clip-5 br-5">Yes delete this subcategory</button>
      </div>
       `;
        PopUp(text)
      }
